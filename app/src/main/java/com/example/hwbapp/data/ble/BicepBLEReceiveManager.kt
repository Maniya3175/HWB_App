...
import com.example.hwbapp.data.BicepResult
...
class BicepBLEReceiveManager @Inject constructor(
    private val bluetoothAdapter: BluetoothAdapter,
    private val context: Context
) : BicepReceiveManager {

    override val data: MutableSharedFlow<Resource<BicepResult>> = MutableSharedFlow()

    private val myBicep = mutableStateOf("B")

    private val DEVICE_NAME = "ES-Tape"
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    ...
                    this@BicepBLEReceiveManager.gatt = gatt
                    ...
                    coroutineScope.launch {
                        data.emit(
                            Resource.Success(
                                data = BicepResult(
                                    "0000",
                                    BicepConnectionState.Disconnected
                                )
                            )
                        )
                    }
        ...
        override fun onMtuChanged(gatt: BluetoothGatt, mtu: Int, status: Int) {
            coroutineScope.launch {
                val characteristic =
                    findCharacteristics(ESTAPE_SERVICE_UIID, ESTAPE_CHARACTERISTICS_UUID)
        ...
        override fun onCharacteristicChanged(
            ...
            when (characteristic.uuid) {
                UUID.fromString(ESTAPE_CHARACTERISTICS_UUID) -> myBicep.value =
                    characteristic?.value!!.decodeToString()

                else -> null
            }

            val bicepResult = BicepResult(myBicep.value, BicepConnectionState.Connected)
            bicepResult?.let {
                coroutineScope.launch {
                    data.emit(
    ...
    override fun closeConnection() {
        ...
        val characteristic = findCharacteristics(ESTAPE_SERVICE_UIID, ESTAPE_CHARACTERISTICS_UUID)
        if(characteristic != null){
